#!/usr/bin/env python3
"""Daily Science_Animal_Amazing_Facts pipeline.

1) Uses OpenAI web search to find science-backed animal fact/discovery stories.
...
def build_story_prompt(
    ...
    return f"""
Give me a comprehensive, up-to-date list of science-backed animal discovery stories involving surprising, counterintuitive, or viral-worthy facts about animal behavior, intelligence, communication, cooperation, social structures, emotions, or evolutionary adaptations.

Focus on stories that feel like:
- Why don't crocodiles eat capybaras?
- Scientists discover elephants use names
- Animals that form unlikely friendships
- Hidden animal behaviors scientists just decoded
- Unexpected biological explanations behind viral animal moments

Include stories where:
- Scientists discovered new forms of communication (names, dialects, signals, calls, gestures).
- Animals show unexpected intelligence, empathy, planning, memory, or social complexity.
- Predator-prey relationships defy expectations.
- Symbiotic or unusual cross-species interactions were scientifically studied.
- Researchers decoded long-misunderstood behaviors.
- Viral animal clips were later explained scientifically.
- New research reveals hidden evolutionary advantages behind strange behavior.
- Scientists identified previously unknown cognitive or emotional traits in animals.

For each story, provide: title/headline, 1-2 sentence summary, source publication, date, direct URL, and category.
Aim for {min_count}-{max_count} distinct stories.
Hard constraints: only animal science facts/behavior/cognition/evolution stories; no generic space, no celebrity animal fluff, no unsupported claims.
...
- Keep strictly animal science context.
""".strip()
...
- Stay on animal science context only.
...
def write_prompt_response_docx(output_dir: Path, stories: list[Story]) -> Path:
    ...
    document = Document()
    document.add_heading("Prompt Response - Animal Science Discovery Stories", level=1)
...
def main(auth_only: bool = False) -> int:
    ...
    load_env_file(repo_root / ".env.science_animal_amazing_facts")
    ...
    output_dir = Path(
        os.getenv(
            "OUTPUT_DIR",
            str(repo_root / "outputs" / "content_research" / "science_animal_amazing_facts"),
        )
    )
